...
function setup() { // 初始值設定
  ...
  for (let i = 0; i < lineCount; i++) {
    let baseColor = color(random(['#005f73', '#ee9b00', '#ae2012', '#9d4edd', '#8ac926'])); // 基本顏色
    baseColor.setAlpha(100); // 設置透明度為 100
...
function drawWavingLine(line) {
  ...
  for (let y = height; y > height - line.height; y -= 10) {
    let offsetX = sin(frameCount * line.frequency + y * 0.05) * map(y, height - line.height, height, 50, 0);
